...
export const AIChatOverlay: React.FC<AIChatOverlayProps> = ({ isOpen, onClose, lang }) => {
  ...
  const handleSend = async (text?: string) => {
    ...
    // 初始化模型回复为空白
    let modelResponse = '';
    const initialMessagesWithModel = [...messagesWithUser, { role: 'model' as const, text: '' }];
    setMessages(initialMessagesWithModel);
    ...
    const responseText = await getAiAssistantResponse(
      ...
      (chunk: string) => {
        // 流式接收并更新消息
        modelResponse += chunk;
        const updatedMessages = [...messagesWithUser, { role: 'model' as const, text: modelResponse }];
    ...
    // 流式结束后，确保最终消息已保存
    const finalMessages = [...messagesWithUser, { role: 'model' as const, text: responseText }];
                              ...
                              <div className="flex gap-3 mt-2 px-1 opacity-0 hover:opacity-100 transition-opacity">
                                  ...
                                  {msg.role === 'model' && (
                                    <>
                                      ...
                                      <button className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all"><ThumbsDown className="h-3.5 w-3.5" /></button>
                                    </>
                                  )}
                              </div>
                          </div>
