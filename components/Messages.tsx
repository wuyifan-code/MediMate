...
export const Messages: React.FC<MessagesProps> = ({ lang }) => {
  ...
  return (
    <div className="pb-20">
      <div className="sticky top-0 z-30 bg-white/85 backdrop-blur-md px-4 py-3 flex items-center justify-between pt-safe border-b border-slate-100 cursor-pointer" onClick={() => window.scrollTo({top: 0, behavior: 'smooth'})}>
         ...
         <div className="flex gap-2">
            ...
            <div className="p-2 hover:bg-slate-100 rounded-full transition-colors cursor-pointer">
                <MailPlus className="h-5 w-5 text-slate-900" />
            </div>
         </div>
      </div>
      ...
      <div className="divide-y divide-slate-100">
         {t.chats.map((chat, idx) => (
           <div key={idx} className={`p-4 hover:bg-slate-50 cursor-pointer transition-colors flex gap-3 ${chat.unread ? 'bg-slate-50/50' : ''}`}>
              <div className="relative">
                 <img src={`https://picsum.photos/60/60?random=${idx + 50}`} className="w-12 h-12 rounded-full object-cover hover:opacity-90 transition-opacity" alt="" />
                 {chat.isOfficial && (
              ...
              <div className="flex-1 overflow-hidden">
                 <div className="flex items-center justify-between mb-0.5">
                    <div className="flex items-center gap-1 truncate">
                       <span className="font-bold text-slate-900 truncate hover:underline">{chat.name}</span>
