...
export const Notifications: React.FC<NotificationsProps> = ({ lang }) => {
  ...
  return (
    <div className="pb-20">
      <div className="sticky top-0 z-30 bg-white/85 backdrop-blur-md border-b border-slate-100 pt-safe cursor-pointer" onClick={() => window.scrollTo({top: 0, behavior: 'smooth'})}>
         ...
         <div className="flex">
            <div 
              className="flex-1 flex justify-center hover:bg-slate-100 cursor-pointer transition-colors relative py-3"
           ...
           <div key={idx} className="p-4 hover:bg-slate-50 cursor-pointer transition-colors flex gap-3">
              ...
              <div className="flex-1">
                 <div className="flex items-center gap-2 mb-1">
                    <img src={`https://picsum.photos/50/50?random=${idx + 20}`} className="w-8 h-8 rounded-full hover:opacity-90 transition-opacity" alt="" />
                 </div>
                 <div className="text-slate-900 text-[15px]">
                    <span className="font-bold hover:underline cursor-pointer">{item.title}</span> <span className="text-slate-500">· {item.time}</span>
                    <div className="text-slate-600 mt-1 leading-normal">{item.content}</div>
                 </div>
              </div>
           </div>
         ))}
