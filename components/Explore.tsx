...
export const Explore: React.FC<ExploreProps> = ({ lang }) => {
  ...
  return (
    <div className="pb-20">
      {/* Search Header */}
      <div className="sticky top-0 z-30 bg-white/85 backdrop-blur-md border-b border-slate-100 pt-safe px-4 py-2 flex items-center gap-4 cursor-pointer" onClick={() => window.scrollTo({top: 0, behavior: 'smooth'})}>
        <div className="flex-1 relative group">
           <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
             <Search className="h-4 w-4 text-slate-500 group-focus-within:text-teal-500" />
           </div>
           <input 
             type="text" 
      ...
      <div className="flex border-b border-slate-100 overflow-x-auto no-scrollbar">
         {[t.forYou, t.trending, t.news, t.entertainment].map((tab, idx) => (
           <div key={idx} className="px-4 py-3 hover:bg-slate-100 cursor-pointer min-w-max relative transition-colors">
      ...
      <div className="border-b border-slate-100">
         ...
         {t.hospitals.map((h, i) => (
           <div key={i} className="px-4 py-3 hover:bg-slate-50 cursor-pointer transition-colors relative">
              <div className="flex justify-between text-xs text-slate-500 mb-0.5">
                 ...
                 <div className="p-1 hover:bg-teal-50 hover:text-teal-500 rounded-full transition-colors -mr-1">
         ...
         <div className="p-4 text-teal-500 text-sm cursor-pointer hover:bg-slate-50 transition-colors">
      ...
      {t.newsItems.map((news, i) => (
        <div key={i} className="flex gap-4 p-4 border-b border-slate-100 hover:bg-slate-50 cursor-pointer transition-colors">
