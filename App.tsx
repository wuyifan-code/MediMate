...
const App: React.FC = () => {
  ...
  const renderRightSidebar = () => {
    return (
      <aside className="hidden lg:block w-[350px] pl-8 py-4 sticky top-0 h-screen overflow-y-auto no-scrollbar">
             ...
             <div className="bg-slate-50 rounded-2xl overflow-hidden border border-slate-100">
                ...
                {[
                  ...
                  <div 
                    ...
                    onClick={() => {
                        handleInteract(`Profile: ${item.handle}`);
             ...
             {/* AI Sidebar Promo */}
             <div className="mt-4 bg-gradient-to-r from-teal-500 to-emerald-500 rounded-2xl p-4 text-white relative overflow-hidden cursor-pointer shadow-lg hover:shadow-xl transition-shadow" onClick={() => setAiChatOpen(true)}>
                <div className="relative z-10 flex items-center gap-3">
                   <div className="bg-white/20 p-2 rounded-full backdrop-blur-sm">
                      <BrainCircuit className="h-6 w-6 text-white" />
                   </div>
                   <div>
                      <div className="font-bold text-lg">MediMate AI</div>
                      <div className="text-white/80 text-sm">{lang === 'zh' ? '智能导诊 & 咨询' : 'Smart Triage & Help'}</div>
                   </div>
                </div>
             </div>
      </aside>
  ...
  const renderMainContent = () => {
    ...
    switch (role) {
      case UserRole.PATIENT:
        return <PatientDashboard lang={lang} onNavigate={handleNavigate} />;
      case UserRole.ESCORT:
        return <EscortDashboard lang={lang} onNavigate={handleNavigate} />;
      default:
            ...
            >
              <div className="flex gap-3">
                <div 
                    ...
                    onClick={(e) => {
                        e.stopPropagation();
                        // Guest promo usually doesn't have a specific profile, but for consistency we can log it
                        handleInteract('View Official Profile');
                    }}
                >
                  <div className="h-10 w-10 bg-black rounded-full flex items-center justify-center text-white font-bold text-sm hover:opacity-90">M</div>
                </div>
                <div>
                  <div className="flex items-center gap-1">
                    <span 
                        className="font-bold text-slate-900 hover:underline cursor-pointer"
                        onClick={(e) => { e.stopPropagation(); handleInteract('View Official Profile'); }}
                    >
                        MediMate 医伴
                    </span>
                  ...
                  <div className="mt-3 rounded-2xl overflow-hidden border border-slate-100 bg-slate-50">
                   ...
                   <div className="mt-3 flex justify-between text-slate-500 max-w-md pr-4">
                     <button className="group flex items-center gap-1 text-xs hover:text-blue-500 transition-colors" onClick={(e) => { e.stopPropagation(); }}>
                         <div className="p-2 rounded-full group-hover:bg-blue-50 transition-colors">
                             <span className="block">💬</span>
                         </div>
                         <span>124</span>
                     </button>
                     <button className="group flex items-center gap-1 text-xs hover:text-green-500 transition-colors" onClick={(e) => { e.stopPropagation(); }}>
                         <div className="p-2 rounded-full group-hover:bg-green-50 transition-colors">
                             <span className="block">⚡</span>
                         </div>
                         <span>502</span>
                     </button>
                     <button className="group flex items-center gap-1 text-xs hover:text-red-500 transition-colors" onClick={(e) => { e.stopPropagation(); }}>
                         <div className="p-2 rounded-full group-hover:bg-red-50 transition-colors">
                             <span className="block">❤️</span>
                         </div>
                         <span>1.3{lang === 'zh' ? '万' : '0k'}</span>
                     </button>
                     <button className="group flex items-center gap-1 text-xs hover:text-teal-500 transition-colors" onClick={(e) => { e.stopPropagation(); }}>
                         <div className="p-2 rounded-full group-hover:bg-teal-50 transition-colors">
                             <span className="block">📊</span>
                         </div>
                         <span>21{lang === 'zh' ? '万' : '0k'}</span>
                     </button>
                     <button className="group flex items-center gap-1 text-xs hover:text-blue-500 transition-colors" onClick={(e) => { e.stopPropagation(); }}>
                         <div className="p-2 rounded-full group-hover:bg-blue-50 transition-colors">
  ...
  return (
    <div className="min-h-screen bg-white text-black font-sans flex justify-center relative">
       ...
       <AIChatOverlay isOpen={aiChatOpen} onClose={() => setAiChatOpen(false)} lang={lang} />

       {/* Mobile Drawer */}
       {mobileDrawerOpen && (
          <div className="fixed inset-0 z-[60] lg:hidden">
             <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setMobileDrawerOpen(false)}></div>
             <div className="absolute left-0 top-0 bottom-0 w-[280px] bg-white shadow-2xl overflow-y-auto pt-safe pb-safe">
       ...
       <div className="fixed bottom-20 right-4 lg:hidden z-40 pb-safe">
          <button 
       ...
       <div className="flex w-full max-w-[1300px]">
          ...
          <main className="flex-1 min-w-0 border-x border-slate-100 max-w-[600px] w-full">
             ...
             {/* Mobile Sticky Header - Only Show on Home Page to allow sub-pages to have their own safe-area headers */}
             {currentPage === 'home' && (
               <div className="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-100 transition-transform duration-200 pt-safe">
       ...
       {/* Mobile Bottom Nav */}
       <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 flex justify-between px-6 pt-3 pb-safe lg:hidden z-50">
         ...
         <div className="cursor-pointer text-slate-900" onClick={() => setAiChatOpen(true)}>
            {/* AI Assistant Icon */}
